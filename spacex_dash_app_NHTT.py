...
def get_pie_chart(value):
    ...
    if value == 'ALL':
        data = filtered_df[filtered_df['class']==1].groupby(['Launch Site'])['Launch Site'].value_counts().reset_index()
        fig = px.pie(data, values='count', 
        names='Launch Site', 
        title='Success & Fail Count')
        return fig
    else:
        data = filtered_df[filtered_df['Launch Site']==value].groupby(['class'])['class'].value_counts().reset_index()
        fig = px.pie(data, values='count', 
        names='class', 
        title='Success & Fail Count')
        return fig
...
def get_scatter_plot(value_site, value_range):
    filtered_df = spacex_df
    low = value_range[0]
    high = value_range[1]
    if value_site == 'ALL':
        ...
        filtered_df = spacex_df[spacex_df['Launch Site']==value_site]
        data = filtered_df[((filtered_df['Payload Mass (kg)'] >= low) & (filtered_df['Payload Mass (kg)'] <= high))]
        fig = px.scatter(data, x="Payload Mass (kg)", y="class", color="Booster Version Category")
        return fig
